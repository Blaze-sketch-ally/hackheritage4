"""Tests for the Student Mentorship Discovery API:
/api/v1/student/mentorship-opportunities.

Adopted from a collaborator branch during the cross-branch integration
pass -- adapted to this project's module names
(app.api.student_mentorship_opportunities /
app.services.student_mentorship_opportunity_service) and route prefix
(distinct from the unrelated /student/mentorships, FACULTY<->STUDENT 1:1
mentorship requests).

Route tests mock app.services.student_mentorship_opportunity_service and
use tests.conftest.authenticated_as. Service tests drive the functions
with a MagicMock Supabase client -- no live project or real token.
Nothing here claims real DB behaviour: RLS on `industry_mentorship` is
the real visibility boundary and is not re-verified against a live
database here.

No migration is added by this file -- a student-facing "mentorship
opportunity" is one PUBLISHED row of the existing `industry_mentorship`
table (031_industry_mentorship.sql). There is no request/pairing table,
so there is no request endpoint to test.
"""
...
from app.services import student_mentorship_opportunity_service as svc
...
_ENDPOINTS = [
    ("get", "/api/v1/student/mentorship-opportunities"),
    ("get", f"/api/v1/student/mentorship-opportunities/{_MID}"),
]
...
def test_list_mentorship_returns_normalized_rows():
    ...
    with (
        ...
        resp = client.get(
            "/api/v1/student/mentorship-opportunities", headers={"Authorization": "Bearer token"}
        )
...
def test_list_mentorship_passes_filters_to_service():
    ...
    with (
        ...
        resp = client.get(
            "/api/v1/student/mentorship-opportunities?work_mode=REMOTE&search=cloud",
...
def test_list_mentorship_no_filters_passes_none():
    ...
    with (
        ...
        client.get("/api/v1/student/mentorship-opportunities", headers={"Authorization": "Bearer token"})
...
def test_list_mentorship_rejects_overlong_search():
    with authenticated_as("STUDENT"):
        resp = client.get(
            "/api/v1/student/mentorship-opportunities?search=" + "x" * 201,
            headers={"Authorization": "Bearer token"},
        )
...
def test_get_mentorship_returns_detail():
    with (
        ...
        resp = client.get(
            f"/api/v1/student/mentorship-opportunities/{_MID}", headers={"Authorization": "Bearer token"}
        )
    assert resp.status_code == 200
    body = resp.json()
...
def test_get_mentorship_404_when_missing_or_not_published():
    with (
        ...
        resp = client.get(
            f"/api/v1/student/mentorship-opportunities/{uuid4()}",
            headers={"Authorization": "Bearer token"},
        )
...
def test_get_mentorship_rejects_non_uuid_id():
    with authenticated_as("STUDENT"):
        resp = client.get(
            "/api/v1/student/mentorship-opportunities/not-a-uuid",
            headers={"Authorization": "Bearer token"},
        )
    assert resp.status_code == 422
...
def test_routes_never_read_an_owner_id_from_the_request():
    from app.api import student_mentorship_opportunities as routes

    for fn in (routes.list_mentorship_opportunities, routes.get_mentorship_opportunity):
...
def test_mentorship_id_param_is_only_a_resource_identifier():
    ...
    for call in re.findall(
        r"student_mentorship_opportunity_service\.\w+\((?:[^()]|\([^()]*\))*\)", code.replace("\n", " ")
    ):
...
def test_service_never_writes_anything():
    """Strictly read-only: the service issues SELECTs and nothing else --
    no insert/update/upsert/delete against any table."""
    ...
    for match in re.finditer(r'\.table\("([a-z_]+)"\)(\.[a-z_]+\()', compact):
        ...
        assert verb not in (".insert(", ".update(", ".upsert(", ".delete("), (
            f"student_mentorship_opportunity_service must not write ({verb} against {match.group(1)})"
        )
...
def test_router_is_registered_under_api_v1_student():
    ...
    assert "/api/v1/student/mentorship-opportunities" in paths
    assert "/api/v1/student/mentorship-opportunities/{mentorship_id}" in paths
    # read-only: the student mentorship-opportunities paths expose only GET
    for path in (
        "/api/v1/student/mentorship-opportunities",
        "/api/v1/student/mentorship-opportunities/{mentorship_id}",
    ):
