"""Tests for Industry project management (Phase 10A): /api/v1/projects.

Route tests mock app.services.industry_project_service and use
tests.conftest.authenticated_as, exactly like tests/test_internships.py
and tests/test_jobs.py. Service tests drive the functions with a
...
from app.api import industry_projects as project_routes
...
def _row(**overrides):
    row = {
        ...
        "description": "Build a recommender for the student portal.",
        "location": "Remote",
        "work_mode": "REMOTE",
        "duration_months": 3,
        "team_size": 4,
...
def _create_body(**overrides):
    body = {"title": "Campus Recommendation Engine", "description": "Build a recommender."}
...
_ENDPOINTS = [
    ...
    ("post", f"/api/v1/projects/{uuid4()}/publish"),
    ("post", f"/api/v1/projects/{uuid4()}/close"),
    ("post", f"/api/v1/projects/{uuid4()}/archive"),
]
...
def test_list_returns_only_callers_projects():
    ...
    with (
        ...
        patch.object(industry_project_service, "list_projects", side_effect=fake_list),
    ):
        resp = client.get(
            "/api/v1/projects?status=PUBLISHED&search=recommendation",
            headers={"Authorization": "Bearer token"},
        )
    assert resp.status_code == 200
    assert captured == {"industry_id": "industry-1", "status": "PUBLISHED", "search": "recommendation"}
    assert resp.json()["projects"][0]["id"] == "project-1"


def test_list_rejects_unknown_status_filter():
    with authenticated_as("INDUSTRY"):
        resp = client.get("/api/v1/projects?status=NONSENSE", headers={"Authorization": "Bearer token"})
...
def test_create_derives_owner_from_token_and_starts_draft():
    ...
    with (
        ...
        patch.object(industry_project_service, "create_project", side_effect=fake_create),
    ):
        resp = client.post(
            "/api/v1/projects",
...
def test_create_rejects_zero_team_size():
    with authenticated_as("INDUSTRY"):
        resp = client.post(
            "/api/v1/projects",
            json=_create_body(team_size=0),
            headers={"Authorization": "Bearer token"},
        )
    assert resp.status_code == 422
...
def test_get_detail_404_when_not_owned():
    with (
        ...
        patch.object(industry_project_service, "get_project", return_value=None) as mock_get,
    ):
        resp = client.get(f"/api/v1/projects/{uuid4()}", headers={"Authorization": "Bearer token"})
...
def test_update_404_when_not_owned():
    with (
        authenticated_as("INDUSTRY", user_id="industry-A"),
        patch.object(industry_project_service, "update_project", return_value=None),
    ):
        resp = client.put(
...
def test_update_passes_owner_id_to_service():
    ...
    def fake_update(_client, industry_id, project_id, data):
        captured["industry_id"] = industry_id
        return _row()

    with (
        authenticated_as("INDUSTRY", user_id="industry-7"),
        patch.object(industry_project_service, "update_project", side_effect=fake_update),
    ):
        resp = client.put(
            f"/api/v1/projects/{uuid4()}",
            json={"title": "Updated"},
...
def test_publish_404_when_not_owned():
    with (
        authenticated_as("INDUSTRY", user_id="industry-A"),
        patch.object(industry_project_service, "publish_project", return_value=None),
    ):
        resp = client.post(
            f"/api/v1/projects/{uuid4()}/publish", headers={"Authorization": "Bearer token"}
        )
    assert resp.status_code == 404
...
def test_publish_missing_fields_maps_to_422():
    with (
        ...
        patch.object(
            ...
            "publish_project",
            side_effect=industry_project_service.PublishValidationError(["location", "work_mode"]),
        ),
    ):
        resp = client.post(
            f"/api/v1/projects/{uuid4()}/publish", headers={"Authorization": "Bearer token"}
...
def test_publish_bad_transition_maps_to_409():
    with (
        ...
        patch.object(
            industry_project_service,
            "publish_project",
            side_effect=industry_project_service.InvalidStatusTransitionError("ARCHIVED", "PUBLISHED"),
        ),
    ):
        resp = client.post(
            f"/api/v1/projects/{uuid4()}/publish", headers={"Authorization": "Bearer token"}
...
def test_close_404_when_not_owned():
    with (
        authenticated_as("INDUSTRY"),
        patch.object(industry_project_service, "close_project", return_value=None),
    ):
        resp = client.post(f"/api/v1/projects/{uuid4()}/close", headers={"Authorization": "Bearer token"})
    assert resp.status_code == 404
...
def test_archive_404_when_not_owned():
    with (
        authenticated_as("INDUSTRY"),
        patch.object(industry_project_service, "archive_project", return_value=None),
    ):
        resp = client.post(
            f"/api/v1/projects/{uuid4()}/archive", headers={"Authorization": "Bearer token"}
        )
    assert resp.status_code == 404
...
def test_archive_bad_transition_maps_to_409():
    with (
        ...
        patch.object(
            industry_project_service,
            "archive_project",
            side_effect=industry_project_service.InvalidStatusTransitionError("ARCHIVED", "ARCHIVED"),
        ),
    ):
        resp = client.post(
            f"/api/v1/projects/{uuid4()}/archive", headers={"Authorization": "Bearer token"}
...
def test_create_forces_draft_and_owner_and_drops_junk():
    ...
    with patch.object(industry_project_service, "get_project", return_value=_row(id="new-1")):
        industry_project_service.create_project(
            supabase,
            "industry-1",
...
def test_update_never_writes_status():
    ...
    with patch.object(
        industry_project_service,
        "get_project",
        side_effect=[_row(id="project-1"), _row(id="project-1", title="Updated")],
    ):
        industry_project_service.update_project(
            supabase, "industry-1", "project-1", {"title": "Updated", "status": "PUBLISHED"}
        )
...
def test_update_returns_none_when_not_owned():
    supabase = MagicMock()
    with patch.object(industry_project_service, "get_project", return_value=None):
        result = industry_project_service.update_project(
            supabase, "industry-1", "project-x", {"title": "Updated"}
        )
...
def test_publish_blocks_when_required_fields_missing():
    supabase = MagicMock()
    with patch.object(
        industry_project_service, "get_project", return_value=_row(location=None, work_mode=None)
    ):
        try:
            industry_project_service.publish_project(supabase, "industry-1", "project-1")
            missing = None
...
def test_publish_sets_status_when_valid():
    ...
    with patch.object(
        ...
        result = industry_project_service.publish_project(supabase, "industry-1", "project-1")
...
def test_publish_rejects_from_archived():
    supabase = MagicMock()
    with patch.object(industry_project_service, "get_project", return_value=_row(status="ARCHIVED")):
        try:
            industry_project_service.publish_project(supabase, "industry-1", "project-1")
            raised = False
        except industry_project_service.InvalidStatusTransitionError:
            raised = True
    assert raised
...
def test_close_only_from_published():
    supabase = MagicMock()
    with patch.object(industry_project_service, "get_project", return_value=_row(status="DRAFT")):
        try:
            industry_project_service.close_project(supabase, "industry-1", "project-1")
            raised = False
        except industry_project_service.InvalidStatusTransitionError:
            raised = True
    assert raised
...
def test_archive_allowed_from_draft():
    ...
    with patch.object(
        ...
        result = industry_project_service.archive_project(supabase, "industry-1", "project-1")
...
def test_archive_rejects_when_already_archived():
    ...
    with patch.object(industry_project_service, "get_project", return_value=_row(status="ARCHIVED")):
        try:
            industry_project_service.archive_project(supabase, "industry-1", "project-1")
            raised = False
        except industry_project_service.InvalidStatusTransitionError:
            raised = True
    assert raised
...
def test_project_modules_do_not_use_service_role():
    assert not hasattr(industry_project_service, "get_supabase")
    assert not hasattr(project_routes, "get_supabase")
    assert hasattr(project_routes, "build_user_client")
