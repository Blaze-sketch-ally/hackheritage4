...
`student_notifications` (059_student_notifications.sql) are the real
...
# ============================================================
# 25-33. Migration 059 schema guard (read from SQL, no live DB)
# ============================================================

_MIGRATIONS_DIR = Path(__file__).resolve().parents[2] / "database" / "migrations"
_M059 = (_MIGRATIONS_DIR / "059_student_notifications.sql").read_text(encoding="utf-8")
_M059_CODE = "\n".join(
    ln for ln in _M059.splitlines() if not ln.strip().startswith("--")
).lower()


def test_migration_059_exists_and_numbering_is_contiguous():
    ...
    assert "059_student_notifications.sql" in names
...
def test_migration_059_creates_one_table_with_pk_and_fk():
    assert "create table if not exists student_notifications" in _M059_CODE
    assert "id uuid primary key default gen_random_uuid()" in _M059_CODE
    assert "student_id uuid not null references profiles (id) on delete cascade" in _M059_CODE


def test_migration_059_type_and_related_entity_constraints():
    assert "check (type in (" in _M059_CODE
    for t in ("application_status", "interview", "assessment", "learning", "mentorship", "event", "system"):
        assert f"'{t}'" in _M059_CODE
    assert "check (related_entity_type in (" in _M059_CODE
    # a pointer is all-or-nothing
    assert "student_notifications_related_entity_paired" in _M059_CODE


def test_migration_059_read_state_column():
    assert "read_at timestamptz" in _M059_CODE
    # notifications are immutable except read_at -> no updated_at column,
    # and set_updated_at is deliberately NOT attached
    assert "updated_at" not in _M059_CODE
    assert "set_updated_at" not in _M059_CODE


def test_migration_059_indexes():
    assert "student_notifications_student_created_idx" in _M059_CODE
    assert "on student_notifications (student_id, created_at desc)" in _M059_CODE
    assert "student_notifications_unread_idx" in _M059_CODE
    assert "where read_at is null" in _M059_CODE


def test_migration_059_rls_enabled_select_and_update_only():
    assert "alter table student_notifications enable row level security" in _M059_CODE
    assert "for select" in _M059_CODE
    assert "for update" in _M059_CODE
    # NO insert policy (system-only writes) and NO delete policy
    assert "for insert" not in _M059_CODE
    assert "for delete" not in _M059_CODE


def test_migration_059_ownership_predicate():
    assert _M059_CODE.count("auth.uid() = student_id and public.is_student(auth.uid())") >= 2


def test_migration_059_freeze_trigger_pins_every_content_column():
    assert "before update on student_notifications" in _M059_CODE
    assert "enforce_student_notification_immutability" in _M059_CODE
    # service_role steps aside, matching 002/023/032
    assert "current_setting('role', true) = 'service_role'" in _M059_CODE
    for col in ("student_id", "type", "title", "body", "related_entity_type", "related_entity_id", "created_at"):
        assert f"new.{col} is distinct from old.{col}" in _M059_CODE
    # read_at is the ONE column NOT frozen
    assert "new.read_at is distinct from old.read_at" not in _M059_CODE


def test_migration_059_is_additive_and_non_destructive():
    assert "drop table" not in _M059_CODE
    for existing in ("profiles", "applications", "assessments", "learning_resources"):
        assert f"alter table {existing} " not in _M059_CODE
    assert "create or replace function public.set_updated_at" not in _M059_CODE


def test_migration_059_not_applied_marker_is_a_human_step():
    """This suite reads the SQL; it does not and cannot apply it. The
    phase contract requires the migration to stay unapplied."""
    assert _M059  # sanity: file is present and non-empty
