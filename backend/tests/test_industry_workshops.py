"""Tests for Industry workshop management (Phase 10C): /api/v1/workshops.

Route tests mock app.services.industry_workshop_service and use
tests.conftest.authenticated_as, exactly like tests/test_industry_training.py.
...
from app.api import industry_workshops as workshop_routes
...
def _row(**overrides):
    row = {
        ...
        "description": "A hands-on half-day session on version control.",
        "location": "Remote",
        "work_mode": "REMOTE",
        "duration_days": 1,
        "capacity": 50,
...
def _create_body(**overrides):
    body = {"title": "Intro to Git Workshop", "description": "A hands-on session."}
...
_ENDPOINTS = [
    ...
    ("get", f"/api/v1/workshops/{uuid4()}"),
    ("put", f"/api/v1/workshops/{uuid4()}"),
    ("post", f"/api/v1/workshops/{uuid4()}/publish"),
    ("post", f"/api/v1/workshops/{uuid4()}/close"),
    ("post", f"/api/v1/workshops/{uuid4()}/archive"),
]
...
def test_list_returns_only_callers_workshops():
    captured = {}
    ...
    with (
        ...
        patch.object(industry_workshop_service, "list_workshops", side_effect=fake_list),
    ):
        resp = client.get(
            "/api/v1/workshops?status=PUBLISHED&search=git",
            headers={"Authorization": "Bearer token"},
        )
    assert resp.status_code == 200
    assert captured == {"industry_id": "industry-1", "status": "PUBLISHED", "search": "git"}
    assert resp.json()["workshops"][0]["id"] == "workshop-1"


def test_list_rejects_unknown_status_filter():
    with authenticated_as("INDUSTRY"):
        resp = client.get("/api/v1/workshops?status=NONSENSE", headers={"Authorization": "Bearer token"})
...
def test_create_derives_owner_from_token_and_starts_draft():
    ...
    with (
        ...
        patch.object(industry_workshop_service, "create_workshop", side_effect=fake_create),
    ):
        resp = client.post(
            "/api/v1/workshops",
...
def test_create_rejects_invalid_duration():
    with authenticated_as("INDUSTRY"):
        resp = client.post(
            "/api/v1/workshops",
            json=_create_body(duration_days=400),
            headers={"Authorization": "Bearer token"},
        )
    assert resp.status_code == 422
...
def test_get_detail_404_when_not_owned():
    with (
        ...
        patch.object(industry_workshop_service, "get_workshop", return_value=None) as mock_get,
    ):
        resp = client.get(f"/api/v1/workshops/{uuid4()}", headers={"Authorization": "Bearer token"})
...
def test_update_404_when_not_owned():
    with (
        authenticated_as("INDUSTRY", user_id="industry-A"),
        patch.object(industry_workshop_service, "update_workshop", return_value=None),
    ):
        resp = client.put(
            f"/api/v1/workshops/{uuid4()}",
...
def test_update_passes_owner_id_to_service():
    ...
    def fake_update(_client, industry_id, workshop_id, data):
        captured["industry_id"] = industry_id
        return _row()

    with (
        authenticated_as("INDUSTRY", user_id="industry-7"),
        patch.object(industry_workshop_service, "update_workshop", side_effect=fake_update),
    ):
        resp = client.put(
            f"/api/v1/workshops/{uuid4()}",
            json={"title": "Updated"},
...
def test_publish_404_when_not_owned():
    with (
        authenticated_as("INDUSTRY", user_id="industry-A"),
        patch.object(industry_workshop_service, "publish_workshop", return_value=None),
    ):
        resp = client.post(
            f"/api/v1/workshops/{uuid4()}/publish", headers={"Authorization": "Bearer token"}
        )
    assert resp.status_code == 404
...
def test_publish_missing_fields_maps_to_422():
    with (
        ...
        patch.object(
            ...
            "publish_workshop",
            side_effect=industry_workshop_service.PublishValidationError(["location", "work_mode"]),
        ),
    ):
        resp = client.post(
            f"/api/v1/workshops/{uuid4()}/publish", headers={"Authorization": "Bearer token"}
...
def test_publish_bad_transition_maps_to_409():
    with (
        ...
        patch.object(
            industry_workshop_service,
            "publish_workshop",
            side_effect=industry_workshop_service.InvalidStatusTransitionError("ARCHIVED", "PUBLISHED"),
        ),
    ):
        resp = client.post(
            f"/api/v1/workshops/{uuid4()}/publish", headers={"Authorization": "Bearer token"}
...
def test_close_404_when_not_owned():
    with (
        authenticated_as("INDUSTRY"),
        patch.object(industry_workshop_service, "close_workshop", return_value=None),
    ):
        resp = client.post(f"/api/v1/workshops/{uuid4()}/close", headers={"Authorization": "Bearer token"})
    assert resp.status_code == 404
...
def test_archive_404_when_not_owned():
    with (
        authenticated_as("INDUSTRY"),
        patch.object(industry_workshop_service, "archive_workshop", return_value=None),
    ):
        resp = client.post(
            f"/api/v1/workshops/{uuid4()}/archive", headers={"Authorization": "Bearer token"}
        )
    assert resp.status_code == 404
...
def test_archive_bad_transition_maps_to_409():
    with (
        ...
        patch.object(
            industry_workshop_service,
            "archive_workshop",
            side_effect=industry_workshop_service.InvalidStatusTransitionError("ARCHIVED", "ARCHIVED"),
        ),
    ):
        resp = client.post(
            f"/api/v1/workshops/{uuid4()}/archive", headers={"Authorization": "Bearer token"}
...
def test_create_forces_draft_and_owner_and_drops_junk():
    ...
    with patch.object(industry_workshop_service, "get_workshop", return_value=_row(id="new-1")):
        industry_workshop_service.create_workshop(
            supabase,
            "industry-1",
...
def test_update_never_writes_status():
    ...
    with patch.object(
        industry_workshop_service,
        "get_workshop",
        side_effect=[_row(id="workshop-1"), _row(id="workshop-1", title="Updated")],
    ):
        industry_workshop_service.update_workshop(
            supabase, "industry-1", "workshop-1", {"title": "Updated", "status": "PUBLISHED"}
        )
...
def test_update_returns_none_when_not_owned():
    supabase = MagicMock()
    with patch.object(industry_workshop_service, "get_workshop", return_value=None):
        result = industry_workshop_service.update_workshop(
            supabase, "industry-1", "workshop-x", {"title": "Updated"}
        )
...
def test_publish_blocks_when_required_fields_missing():
    supabase = MagicMock()
    with patch.object(
        industry_workshop_service, "get_workshop", return_value=_row(location=None, work_mode=None)
    ):
        try:
            industry_workshop_service.publish_workshop(supabase, "industry-1", "workshop-1")
            missing = None
...
def test_publish_sets_status_when_valid():
    ...
    with patch.object(
        ...
        result = industry_workshop_service.publish_workshop(supabase, "industry-1", "workshop-1")
...
def test_publish_rejects_from_archived():
    supabase = MagicMock()
    with patch.object(industry_workshop_service, "get_workshop", return_value=_row(status="ARCHIVED")):
        try:
            industry_workshop_service.publish_workshop(supabase, "industry-1", "workshop-1")
            raised = False
        except industry_workshop_service.InvalidStatusTransitionError:
            raised = True
    assert raised
...
def test_close_only_from_published():
    supabase = MagicMock()
    with patch.object(industry_workshop_service, "get_workshop", return_value=_row(status="DRAFT")):
        try:
            industry_workshop_service.close_workshop(supabase, "industry-1", "workshop-1")
            raised = False
        except industry_workshop_service.InvalidStatusTransitionError:
            raised = True
    assert raised
...
def test_archive_allowed_from_draft():
    ...
    with patch.object(
        ...
        result = industry_workshop_service.archive_workshop(supabase, "industry-1", "workshop-1")
...
def test_archive_rejects_when_already_archived():
    ...
    with patch.object(industry_workshop_service, "get_workshop", return_value=_row(status="ARCHIVED")):
        try:
            industry_workshop_service.archive_workshop(supabase, "industry-1", "workshop-1")
            raised = False
        except industry_workshop_service.InvalidStatusTransitionError:
            raised = True
    assert raised
...
def test_workshop_modules_do_not_use_service_role():
    assert not hasattr(industry_workshop_service, "get_supabase")
    assert not hasattr(workshop_routes, "get_supabase")
    assert hasattr(workshop_routes, "build_user_client")
