...
# NOTE: the source branch also had a test here asserting the shape of the
# /api/v1/student/notifications routes. Those routes (the full
# student_notifications feature -- app.api.student_notifications) were
# deliberately NOT adopted as part of this integration pass -- only this
# producer (which safely no-ops via contextlib.suppress if the table/
# routes don't exist yet, per its own docstring) was pulled forward since
# app.api.applications depends on it. That test is deferred along with
# the rest of the student_notifications feature to a later phase.
