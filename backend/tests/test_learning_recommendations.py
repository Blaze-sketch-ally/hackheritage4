...
pass and adapted: the source branch computed gap_skills from one
persisted "target job role" via its own skill_gap_service (PERSONAL /
JOB_ROLE dispatch). This project has no such persisted target, so
app.services.skill_recommendation_service.get_aggregate_skill_gaps
(aggregating GAP/NOT_ASSESSED skills across every career role in this
project's own catalog) replaces that dispatch, always as one mode
("AGGREGATE") -- see that module's own docstring. Sections A, B, C below
are adapted for that; sections D-F (the adapter itself, which never knew
or cared where gap_skills came from) are unchanged.

Route tests mock app.services.skill_recommendation_service and
app.services.learning_recommendation_service, and use
tests.conftest.authenticated_as -- exactly like tests/test_student_learning.py.
Service tests drive the adapter with a MagicMock Supabase client. Nothing
here re-verifies live RLS.
"""
...
def test_route_ignores_client_supplied_skill_and_student_id_query_params():
    ...
    def fake_gaps(_client, student_id):
        captured["gap_student_id"] = student_id
        return [_rec(_SKILL_A, "Python")]

    def fake_get(_client, student_id, gap_skills):
        captured["rec_student_id"] = student_id
        captured["skill_ids"] = [s["skill_id"] for s in gap_skills]
        return []

    with (
        authenticated_as("STUDENT", user_id="student-real"),
        patch.object(skill_recommendation_service, "get_aggregate_skill_gaps", side_effect=fake_gaps),
    ...
    assert captured["gap_student_id"] == "student-real"
    assert captured["rec_student_id"] == "student-real"
    assert captured["skill_ids"] == [_SKILL_A]  # from the engine, not the query string


# ============================================================
# C. Aggregate skill-gap integration -- consumes the canonical engine
# ============================================================


def test_aggregate_mode_consumes_get_aggregate_skill_gaps_output():
    captured = {}
    ...
    with (
        ...
        patch.object(skill_recommendation_service, "get_aggregate_skill_gaps", return_value=recs),
        patch.object(rec_svc, "get_recommended_resources", side_effect=fake_get),
    ):
        resp = client.get(_URL, headers={"Authorization": "Bearer token"})
    assert resp.status_code == 200
    assert resp.json()["mode"] == "AGGREGATE"
    assert captured["gap_skills"] == recs


def test_route_does_not_reimplement_the_gap_engine():
    """The route dispatches to skill_recommendation_service; the adapter
    (learning_recommendation_service) never imports it and computes no
    proficiency/readiness/status of its own."""
    adapter_src = _code_only(rec_svc)
    assert "skill_recommendation_service" not in adapter_src
    ...
    assert "skill_recommendation_service.get_aggregate_skill_gaps" in route_src
...
def test_response_matched_skills_are_trimmed_to_four_fields():
    ...
    with (
        authenticated_as("STUDENT"),
        patch.object(skill_recommendation_service, "get_aggregate_skill_gaps", return_value=[]),
...
def test_no_import_cycle_between_adapter_and_gap_service():
    assert "learning_recommendation_service" not in _code_only(skill_recommendation_service)
    assert "skill_recommendation_service" not in _code_only(rec_svc)
