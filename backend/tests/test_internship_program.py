...
RLS (060_internship_program.sql, via public.owns_internship_program +
        ...
        if self._op == "insert":
            ...
            for p in payloads:
                ...
                if self._table == "program_assignments" and "program_id" not in r:
                    # emulates the set_program_assignment_program_id trigger (060)
                    mod = next(
...
def test_no_assignment_delete_helper_exists():
    # 060 grants no DELETE policy on program_assignments -- hide via is_published.
