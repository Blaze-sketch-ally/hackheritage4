...
(database/migrations/081_job_training.sql).
...
def _index_of(needle: str) -> int:
    i = M052_C.find(needle)
    assert i != -1, f"not found in 052: {needle!r}"
    return i
...
def _fn_signature(name: str) -> str:
    after = M052_C.split(f"create or replace function public.{name}", 1)[1]
...
def test_migration_052_exists():
    assert M052.is_file(), f"expected {M052} to exist"
...
def test_migration_numbering_stays_contiguous_and_unique():
    ...
    assert 81 in numbers  # 081_job_training on the merged branch
    ...
    assert 82 in numbers and 83 in numbers  # job_training_completion + interview visibility


def test_no_historical_migration_was_modified_by_j1():
    """J1 edits none of the pre-existing migrations. (The Internship
    Workspace chain was renumbered 037/038/039 -> 049/050/051 when the
    institution-tenancy lineage claimed 037-048; its content is unchanged,
    so the same create-table markers are asserted at the new paths.)"""
    for anchor, marker in (
        ("055_applications.sql", "create table applications"),
        ("054_jobs.sql", "create table if not exists jobs"),
        ("064_learning_resources.sql", "create table if not exists learning_resources"),
        ("059_student_notifications.sql", "create table if not exists student_notifications"),
        ("060_internship_program.sql", "create table if not exists internship_programs"),
        ("061_internship_workspace.sql", "create table if not exists internship_workspaces"),
        ("062_workspace_submissions_completion.sql", "create table if not exists workspace_submissions"),
    ):
        assert marker in _raw(MIGRATIONS_DIR / anchor)


def test_j1_is_additive_and_non_destructive():
    assert "drop table" not in M052_L, "052 must not drop a table"
    assert "truncate" not in M052_L, "052 must not truncate"
    for line in M052_L.splitlines():
        s = line.strip()
        if s.startswith("drop ") and "execute format(" not in s:
            assert s.startswith(("drop policy if exists", "drop trigger if exists")), (
                f"052: unexpected drop statement: {s!r}"
            )
...
def test_j1_does_not_touch_the_frozen_existing_tables():
    ...
    assert "alter table public.student_notifications" in M052_L
    frozen = (
    ...
    for t in frozen:
        assert f"alter table {t} " not in M052_L, f"052 must not alter {t}"
        assert f"alter table public.{t} " not in M052_L, f"052 must not alter public.{t}"
        assert f"create table if not exists {t}" not in M052_L, f"052 must not (re)create {t}"


def test_j1_does_not_add_a_new_application_status():
    assert "alter table applications" not in M052_L
    assert "alter table public.applications" not in M052_L
    # 052 only ever READS applications.status, never widens/edits its CHECK.
    assert "status in ('applied'" not in M052_L
    assert "add constraint applications" not in M052_L


def test_shared_helpers_are_reused_not_redefined():
    for fn in ("set_updated_at", "is_student", "is_industry"):
        assert f"create or replace function public.{fn}" not in M052_C
    assert "execute procedure public.set_updated_at()" in M052_C
    assert "public.is_student(auth.uid())" in M052_C
    assert "public.is_industry(auth.uid())" in M052_C


def test_j1_never_references_student_skills_or_verification():
    assert "student_skills" not in M052_L, "052 must not reference student_skills"
    assert "is_verified" not in M052_L, "052 must not touch skill verification"
    assert "score_assessment_attempt" not in M052_L
    # training completion is NOT skill evidence
    assert "student_learning_progress" not in M052_L
...
def test_j1_does_not_repurpose_student_learning_or_industry_training():
    for other in ("learning_resources", "student_learning_progress", "industry_training"):
        assert f"references {other}" not in M052_L
        assert f"from public.{other}" not in M052_L
        assert f"join public.{other}" not in M052_L
...
def test_all_six_tables_are_created():
    for t in ALL_NEW_TABLES:
        assert f"create table if not exists {t}" in M052_L, f"missing table {t}"


def test_rls_enabled_on_every_new_table():
    for t in ALL_NEW_TABLES:
        assert f"alter table {t} enable row level security" in M052_L, f"RLS not enabled on {t}"


def test_no_new_table_has_a_delete_policy():
    for t in ALL_NEW_TABLES:
        assert f"on {t} for delete" not in M052_L, f"{t} must not allow hard delete"
...
def test_job_program_is_strictly_one_per_job():
    block = _table_block(M052_L, "job_programs")
...
def test_job_program_has_no_internship_reference_anywhere_in_the_migration():
    assert "internship_id" not in M052_L, "052 must have NO internship_id column"
    assert "references internships" not in M052_L
    assert "internship_programs" not in M052_L
...
def test_job_program_status_lifecycle_values():
    assert _check_set(_table_block(M052_C, "job_programs"), "status") == {
...
def test_owns_job_program_is_locked_down_then_granted_to_authenticated():
    assert "revoke all on function public.owns_job_program(uuid) from public" in M052_C
    assert "revoke all on function public.owns_job_program(uuid) from anon" in M052_C
    assert "grant execute on function public.owns_job_program(uuid) to authenticated" in M052_C
...
def test_job_program_modules_shape():
    block = _table_block(M052_L, "job_program_modules")
    ...
    assert "job_program_modules_program_id_idx" in M052_L


def test_job_program_items_are_one_normalized_table_with_a_type_check():
    block = _table_block(M052_C, "job_program_items")
    ...
    assert "job_program_items_module_id_idx" in M052_L


def test_job_program_skills_reference_canonical_catalog_with_required_optional():
    block = _table_block(M052_C, "job_program_skills")
    bl = block.lower()
    ...
    assert "job_program_skills_skill_id_idx" in M052_L
    # mutable child content -> `for all` owner policy (replaceable set)
    assert "on job_program_skills for all" in M052_L
...
def test_job_program_assignments_is_one_table_not_three():
    for forbidden in ("create table if not exists job_assignments",
        ...
        assert forbidden not in M052_L
    block = _table_block(M052_C, "job_program_assignments")
...
def test_job_program_assignment_program_id_is_derived_by_a_before_trigger():
    assert "create or replace function public.set_job_program_assignment_program_id" in M052_C
    assert "before insert or update on job_program_assignments" in M052_C
...
def test_program_record_tables_have_only_select_insert_update_policies():
    for t in ("job_programs", "job_program_modules", "job_program_items",
              "job_program_assignments"):
        assert f"on {t} for delete" not in M052_L, f"{t} must not allow delete"
        assert f"on {t} for all" not in M052_L, f"{t} must not have a `for all` policy"
        for verb in ("select", "insert", "update"):
            assert f"on {t} for {verb}" in M052_L, f"{t} needs a `for {verb}` policy"
...
def test_enrollment_is_one_per_application_and_keyed_to_the_application():
    block = _table_block(M052_L, "job_training_enrollments")
...
def test_enrollment_status_uses_the_minimum_lifecycle():
    assert _check_set(_table_block(M052_C, "job_training_enrollments"), "enrollment_status") == {
        "ACTIVE", "COMPLETED", "REVOKED"
    }
    # no student opt-in step was invented
    assert "ACCEPTED" not in M052_C
    assert "DECLINED" not in M052_C
    assert "PENDING_ACCEPTANCE" not in M052_C
...
def test_enrollment_derivation_trigger_exists_and_is_before_insert():
    assert "create or replace function public.set_job_training_enrollment_derived_ids" in M052_C
    assert "before insert on job_training_enrollments" in M052_C
...
def test_enrollment_indexes():
    assert "job_training_enrollments_student_status_idx" in M052_L
    assert "job_training_enrollments_industry_status_idx" in M052_L
    assert "job_training_enrollments_job_id_idx" in M052_L
...
def test_enrollment_identity_freeze_trigger_pins_every_identity_column():
    assert "create or replace function public.prevent_job_training_enrollment_identity_change" in M052_C
    assert "before update on job_training_enrollments" in M052_C
...
def test_enrollment_transition_trigger_documents_and_enforces_the_lifecycle():
    assert "create or replace function public.enforce_job_training_enrollment_transitions" in M052_C
...
def test_enrollment_has_no_student_update_or_delete_policy():
    block = M052_L.split("alter table job_training_enrollments enable row level security", 1)[1]
...
def test_student_access_helper_is_locked_down_then_granted_to_authenticated():
    assert "revoke all on function public.student_can_access_job_program(uuid) from public" in M052_C
    assert "revoke all on function public.student_can_access_job_program(uuid) from anon" in M052_C
    assert (
...
def test_student_content_policies_use_the_helper_and_require_published():
    for t in PROGRAM_TABLES:
        assert f"on {t} for select" in M052_L, f"052 must add a student SELECT policy on {t}"
    assert "public.student_can_access_job_program" in M052_C
    assert "job_program_modules.is_published = true" in M052_C
    assert "job_program_items.is_published = true" in M052_C
    assert "job_program_assignments.is_published = true" in M052_C
...
def test_no_new_policy_uses_using_true():
    ...
    assert "using (true)" not in M052_L
    assert "with check (true)" not in M052_L
    assert "for select\n  to authenticated\n  using (true)" not in M052_L
...
def test_every_policy_targets_authenticated_only():
    ...
    assert "for select\n  to anon" not in M052_L
    assert " to public" not in M052_L.replace("from public", "").replace("public.", "")
    assert "to anon, authenticated" not in M052_L
...
def test_industry_content_policies_route_through_owns_job_program():
    ...
    for t in ("job_program_modules", "job_program_items", "job_program_skills",
              "job_program_assignments"):
        section = M052_L.split(f"alter table {t} enable row level security", 1)[1]
...
def test_language_sql_helpers_only_reference_already_created_tables():
    ...
    created: set[str] = set()
    chunks = re.split(
        r"(create table if not exists \w+|create or replace function public\.\w+)", M052_C
    )
    idx = 0
...
def test_every_function_is_created_before_it_is_referenced_by_a_policy_or_trigger():
    for fn in ("owns_job_program", "student_can_access_job_program",
        ...
        define_at = M052_C.find(f"create or replace function public.{fn}")
...
def test_triggers_are_created_after_their_table_and_function():
    for trig, tbl, fn in (
        ...
        trig_at = M052_C.find(f"create trigger {trig}")
        assert trig_at != -1, f"missing trigger {trig}"
        assert M052_C.find(f"create table if not exists {tbl}") < trig_at
        if fn != "set_updated_at":  # set_updated_at is pre-existing (012)
            assert M052_C.find(f"create or replace function public.{fn}") < trig_at
...
def test_all_trigger_typed_functions_revoke_from_public():
    for fn in ("set_job_program_assignment_program_id",
               ...
               "prevent_job_training_enrollment_identity_change",
               "enforce_job_training_enrollment_transitions"):
        assert f"revoke all on function public.{fn}() from public" in M052_C
...
def test_notification_widening_resolves_the_constraint_name_defensively():
    assert M052_C.count("do $$") >= 2
    assert "array_length(con.conkey, 1) = 1" in M052_C
    assert "att.attname = 'type'" in M052_C
    assert "att.attname = 'related_entity_type'" in M052_C
    # the 2-column *_paired check is never named / dropped
    assert "student_notifications_related_entity_paired" not in M052_C


def test_notification_table_is_not_recreated_or_bare_altered():
    assert "create table if not exists student_notifications" not in M052_L
    assert "alter table public.student_notifications\n  drop constraint" not in M052_L
    assert "alter table student_notifications drop constraint" not in M052_L
    # the only two add-constraint statements in the file are the widenings
    added = re.findall(r"add constraint (\w+)", M052_C)
    assert set(added) == {
...
def test_no_notification_producer_is_wired_in_j1():
    ...
    assert "emit_" not in M052_L
    assert "notification_producer" not in M052_L
    assert "insert into student_notifications" not in M052_L
    assert "insert into public.student_notifications" not in M052_L
...
def test_migration_has_no_obvious_sql_syntax_smells():
    ...
    assert M052_RAW.count("$$") % 2 == 0, "unbalanced $$ dollar-quote blocks"
    # every `create table if not exists` block is closed with `);`
    for t in ALL_NEW_TABLES:
        block = _table_block(M052_RAW, t)
        assert block.count("(") >= block.count(")") - 1  # rough balance inside the block
    # no accidental reference to the internship helper
    assert "student_can_access_program(" not in M052_C
    assert "owns_internship_program(" not in M052_C
...
def test_completed_and_revoked_columns_cannot_be_set_on_an_active_row():
    block = _table_block(M052_C, "job_training_enrollments")
...
def test_j1_migration_stays_pure_schema_no_app_logic_embedded():
    ...
    guarantees is that migration 052 itself contains no application logic
    ...
    for smell in ("fastapi", "APIRouter", "require_student", "require_industry",
        ...
        assert smell not in M052_C, f"052 migration must not embed app code: {smell!r}"
