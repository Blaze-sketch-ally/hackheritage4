...
def test_phase6_needs_no_new_migration_submission_reviews_already_has_the_columns():
    ...
    # No Phase 6 migration was added for THIS domain beyond its own chain
    # tip (051). Migrations 040-048 are the unrelated institution-tenancy
    # lineage (arunangshu-part1) that this chain was renumbered to follow --
    # their existence is expected and is not a Phase 6 addition.
    later = [int(p.name[:3]) for p in MIGRATIONS_DIR.glob("[0-9][0-9][0-9]_*.sql") if int(p.name[:3]) >= 52]
    assert later == [], f"unexpected post-051 migration(s): {later}"
