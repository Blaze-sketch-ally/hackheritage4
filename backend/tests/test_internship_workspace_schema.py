...
061_internship_workspace.sql, 062_workspace_submissions_completion.sql).
...
M060 = MIGRATIONS_DIR / "060_internship_program.sql"
M061 = MIGRATIONS_DIR / "061_internship_workspace.sql"
M062 = MIGRATIONS_DIR / "062_workspace_submissions_completion.sql"
...
def test_every_language_sql_helper_body_only_references_existing_tables():
    created: set[str] = set()
    for sql in (M060_C, M061_C, M062_C):
...
def test_workspace_child_policies_come_after_the_ownership_helpers():
    # 061: student_owns_workspace / industry_owns_workspace must be defined
    ...
    for helper in ("student_owns_workspace", "industry_owns_workspace",
        ...
        assert _created_before(
            ...
            f"public.{helper}(",
        ) or M061_C.count(f"public.{helper}(") == 1, (
            f"{helper} is used before it is defined in 061"
        )
...
def test_no_historical_migration_was_modified():
    """This integration pass adds the internship-workspace migrations and
    edits no pre-existing migration file. Anchors below reflect THIS repo's
    own migration numbering (adopted from a collaborator branch and
    renumbered to fit this repo's sequence -- see 053/055/059's own
    headers), not the source branch's numbering."""
    names = {p.name for p in MIGRATIONS_DIR.glob("[0-9][0-9][0-9]_*.sql")}
    for anchor, marker in (
        ("053_internships.sql", "create table if not exists internships"),
        ("055_applications.sql", "create table applications"),
        ("059_student_notifications.sql", "create table if not exists student_notifications"),
        ("003_skills.sql", "create table if not exists skills"),
        ("014_score_assessment_attempt.sql", "score_assessment_attempt"),
    ):
...
def test_phase1_migrations_are_additive_and_non_destructive():
    for name, sql in (("060", M060_L), ("061", M061_L), ("062", M062_L)):
    ...
    # student_notifications CHECK widening, isolated in 062.
    assert "alter table public.student_notifications" in M062_L
...
def test_shared_helpers_are_reused_not_redefined():
    ...
    assert "execute procedure public.set_updated_at()" in M060_C
    assert "public.is_student(auth.uid())" in M061_C
    assert "public.is_industry(auth.uid())" in M060_C
...
def test_all_twelve_tables_are_created():
    for t in PROGRAM_TABLES:
        assert f"create table if not exists {t}" in M060_L
    for t in WORKSPACE_TABLES:
        assert f"create table if not exists {t}" in M061_L
    for t in SUBMISSION_TABLES:
        assert f"create table if not exists {t}" in M062_L


def test_rls_enabled_on_every_new_table():
    for t in PROGRAM_TABLES:
        assert f"alter table {t} enable row level security" in M060_L
    for t in WORKSPACE_TABLES:
        assert f"alter table {t} enable row level security" in M061_L
    for t in SUBMISSION_TABLES:
        assert f"alter table {t} enable row level security" in M062_L
...
def test_internship_program_is_strictly_one_per_internship():
    block = _table_block(M060_L, "internship_programs")
...
def test_program_status_lifecycle_values():
    assert _check_set(_table_block(M060_C, "internship_programs"), "status") == {
...
def test_module_items_are_one_normalized_table_with_a_type_check():
    block = _table_block(M060_C, "module_items")
...
def test_program_skills_reference_canonical_catalog_with_required_optional():
    block = _table_block(M060_C, "program_skills")
    bl = block.lower()
...
def test_program_assignments_is_one_table_not_three():
    ...
    assert _check_set(_table_block(M060_C, "program_assignments"), "assignment_type") == {
        "ASSIGNMENT", "QUIZ", "PROJECT"
    }


def test_program_assignments_supports_project_and_quiz_fields():
    block = _table_block(M060_C, "program_assignments")
    bl = block.lower()
    ...
    assert "set_program_assignment_program_id" in M060_C
...
def test_program_ownership_resolves_through_internship_ownership():
    assert "create or replace function public.owns_internship_program" in M060_C
    assert "join public.internships intr on intr.id = prog.internship_id" in M060_C
    assert "intr.industry_id = auth.uid()" in M060_C
    assert "public.is_industry(auth.uid())" in M060_C
...
def test_program_record_tables_have_no_delete_or_all_policy():
    for t in ("internship_programs", "program_modules", "module_items", "program_assignments"):
        assert f"on {t} for delete" not in M060_L, f"{t} must not allow hard delete"
        assert f"on {t} for all" not in M060_L, f"{t} must not have a `for all` policy"
        for verb in ("select", "insert", "update"):
            assert f"on {t} for {verb}" in M060_L, f"{t} needs a `for {verb}` policy"
...
def test_program_skills_is_a_replaceable_child_table():
    ...
    assert "on program_skills for all" in M060_L


def test_students_do_not_get_program_policies_in_060():
    assert "student_can_access_program" not in M060_C
    assert '"students can' not in M060_L
...
def test_workspace_is_one_per_application_and_keyed_correctly():
    block = _table_block(M061_L, "internship_workspaces")
...
def test_workspace_work_mode_snapshot_is_remote_or_hybrid_only():
    assert _check_set(_table_block(M061_C, "internship_workspaces"), "work_mode") == {
        "REMOTE", "HYBRID"
    }


def test_workspace_status_uses_the_approved_lifecycle():
    assert _check_set(_table_block(M061_C, "internship_workspaces"), "workspace_status") == {
...
def test_workspace_identity_and_transition_guards_exist():
    assert "create or replace function public.set_workspace_derived_ids" in M061_C
    assert "create or replace function public.prevent_workspace_identity_change" in M061_C
    assert "create or replace function public.enforce_workspace_status_transitions" in M061_C
    assert "v_app_status <> 'SELECTED'" in M061_C
    assert "v_work_mode not in ('REMOTE', 'HYBRID')" in M061_C
    for col in ("application_id", "student_id", "industry_id", "internship_id", "work_mode"):
        assert f"new.{col} is distinct from old.{col}" in M061_C
    assert "current_setting('role', true) = 'service_role'" in M061_C
    assert "new.workspace_status in ('ACCEPTED', 'DECLINED')" in M061_C


def test_workspace_has_no_delete_policy():
    assert "on internship_workspaces for delete" not in M061_L
    assert "on internship_workspaces for all" not in M061_L
    for verb in ("select", "insert", "update"):
        assert f"on internship_workspaces for {verb}" in M061_L
...
def test_workspace_rls_never_depends_on_internship_published_status():
    body = M061_C.split("function public.student_can_access_program", 1)[1].split("$$", 2)[1].lower()
...
def test_workspace_child_isolation_uses_ownership_helpers():
    assert "create or replace function public.student_owns_workspace" in M061_C
    assert "create or replace function public.industry_owns_workspace" in M061_C
    assert "w.student_id = auth.uid()" in M061_C
    assert "w.industry_id = auth.uid()" in M061_C


def test_student_program_content_policies_are_added_in_061_and_require_published():
    for t in ("internship_programs", "program_modules", "module_items",
              "program_assignments", "program_skills"):
        assert f"on {t} for select" in M061_L, f"061 must add a student SELECT policy on {t}"
    assert "program_modules.is_published = true" in M061_C
    assert "module_items.is_published = true" in M061_C
    assert "program_assignments.is_published = true" in M061_C
    assert "public.student_can_access_program" in M061_C
...
def test_workspace_skill_selections_shape_and_uniqueness():
    block = _table_block(M061_L, "workspace_skill_selections")
    ...
    assert "on workspace_skill_selections for update" not in M061_L
    assert "on workspace_skill_selections for delete" in M061_L
    assert "ps.requirement = 'OPTIONAL'" in M061_C


# ============================================================
# 9. Work-mode protection trigger (in 061, where internship_workspaces exists)
# ============================================================


def test_workmode_protection_trigger_blocks_only_the_orphaning_transition():
    assert "create or replace function public.prevent_ineligible_workmode_change" in M061_C
    assert "before update on internships" in M061_C
    body = M061_C.split("function public.prevent_ineligible_workmode_change", 1)[1].split("$$", 2)[1]
...
def test_submissions_are_append_only_with_attempt_numbers():
    block = _table_block(M062_C, "workspace_submissions")
    bl = block.lower()
    ...
    assert "RESUBMITTED" not in M062_C
    assert "'DRAFT'" not in block
    assert "create or replace function public.prevent_workspace_submission_content_change" in M062_C
    assert "a submission is append-only" in M062_L
    assert "on workspace_submissions for delete" not in M062_L


def test_submission_attempt_number_is_server_assigned():
    assert "create or replace function public.set_workspace_submission_attempt_number" in M062_C
    assert "coalesce(max(s.attempt_number), 0) + 1" in M062_C
    assert "v_prev_status not in ('REVISION_REQUESTED', 'REJECTED')" in M062_C


def test_submission_reviews_are_the_source_of_truth_and_immutable():
    block = _table_block(M062_C, "submission_reviews")
    bl = block.lower()
    ...
    assert "new.reviewer_id := auth.uid()" in M062_C
    assert "on submission_reviews for update" not in M062_L
    assert "on submission_reviews for delete" not in M062_L
    assert "on submission_reviews for select" in M062_L
    assert "on submission_reviews for insert" in M062_L
...
def test_completion_is_one_per_workspace_and_needs_explicit_verification():
    block = _table_block(M062_C, "internship_completions")
    bl = block.lower()
    ...
    assert "new.verified_by := auth.uid()" in M062_C
...
def test_certificate_is_one_to_one_with_a_passed_completion():
    block = _table_block(M062_C, "internship_certificates")
    bl = block.lower()
    ...
    assert "a certificate can only be issued for a passed internship completion" in M062_L


def test_certificate_number_format_is_server_generated():
    assert "create or replace function public.generate_internship_certificate_number" in M062_C
    assert "'AIC-INT-' || to_char(now(), 'YYYY') || '-'" in M062_C
    assert "ABCDEFGHIJKLMNOPQRSTUVWXYZ234567" in M062_C
    assert "for v_i in 1..13 loop" in M062_C
    assert "gen_random_bytes" not in M062_C
    assert "revoke all on function public.generate_internship_certificate_number() from authenticated" in M062_C


def test_certificate_is_immutable_except_pdf_and_revocation():
    assert "create or replace function public.prevent_internship_certificate_tamper" in M062_C
    for frozen in ("completion_id", "certificate_number", "issued_at", "details",
                   "student_id", "industry_id", "internship_id"):
        assert f"new.{frozen} is distinct from old.{frozen}" in M062_C
    assert "on internship_certificates for delete" not in M062_L
...
def test_verify_function_is_security_definer_with_pinned_search_path():
    after = M062_C.split("create or replace function public.verify_internship_certificate", 1)[1]
...
def test_verify_function_is_granted_to_anon_for_public_verification():
    assert "revoke all on function public.verify_internship_certificate(text) from public" in M062_C
    assert ("grant execute on function public.verify_internship_certificate(text) "
            "to anon, authenticated") in M062_C
    # no table-level anon/public policy on internship_certificates
    cert_rls = M062_L.split("alter table internship_certificates enable row level security", 1)[1]
...
def test_stipend_is_one_per_workspace_with_a_terminal_lifecycle():
    block = _table_block(M062_C, "stipend_disbursements")
    bl = block.lower()
    assert "workspace_id uuid not null references internship_workspaces (id) on delete cascade" in bl
    assert "unique (workspace_id)" in bl
    assert _check_set(block, "disbursement_status") == {"PENDING", "APPROVED", "RELEASED", "CANCELLED"}
    assert "a released or cancelled stipend record cannot change state" in M062_L
    assert "old.disbursement_status in ('RELEASED', 'CANCELLED')" in M062_C


def test_stipend_student_is_read_only():
    block = M062_L.split("alter table stipend_disbursements enable row level security", 1)[1]
...
def test_notification_widening_resolves_the_constraint_name_defensively():
    assert M062_C.count("do $$") >= 2
    assert "array_length(con.conkey, 1) = 1" in M062_C
    assert "att.attname = 'type'" in M062_C
    assert "att.attname = 'related_entity_type'" in M062_C
    # the 2-column *_paired check is never named / dropped
    assert "student_notifications_related_entity_paired" not in M062_C


def test_notification_table_is_not_recreated():
    tail = M062_L.split("student_notifications", 1)[1]
    ...
    assert "alter table public.student_notifications\n  drop constraint" not in M062_L
    assert "alter table student_notifications drop constraint" not in M062_L
