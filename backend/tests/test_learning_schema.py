...
MIGRATION_064 = MIGRATIONS_DIR / "064_learning_resources.sql"
SEED_064 = SEED_DIR / "learning_resources.sql"
...
def test_migration_064_exists():
    assert MIGRATION_064.is_file(), f"expected {MIGRATION_064} to exist"
...
def test_migration_numbering_stays_contiguous_and_unique():
    ...
    assert 64 in numbers, "064 (learning) must still exist"
    # 064 was the highest when this suite was written; new forward
...
def test_migration_064_does_not_reuse_a_historical_number_or_rename_anything():
    names = {p.name for p in MIGRATIONS_DIR.glob("[0-9][0-9][0-9]_*.sql")}
    assert "064_learning_resources.sql" in names
    ...
    # anchor historical files still present, unrenamed -- these are THIS
    # repo's own historical migrations, not the source branch's (which had
    # a different, retired job_roles/skill_gap history at these numbers).
    for anchor in ("003_skills.sql", "022_career_roles_skill_gap.sql", "002_protect_admin_role.sql"):
        assert anchor in names


def test_migration_064_creates_all_three_tables():
    sql = _sql_without_comments(MIGRATION_064).lower()
    for table in LEARNING_TABLES:
        assert f"create table if not exists {table}" in sql, f"064 must create {table}"
...
def test_learning_resources_core_fields_and_constraints():
    sql = _sql_without_comments(MIGRATION_064)
...
def test_migration_064_does_not_reference_forbidden_tables():
    sql = _sql_without_comments(MIGRATION_064).lower()
    for forbidden in (
        "student_skills",
        "career_roles",
        ...
        assert forbidden not in sql, f"064 must not reference {forbidden}"


def test_migration_064_has_no_destructive_statements():
    sql = _sql_without_comments(MIGRATION_064).lower()
    ...
    for line in sql.splitlines():
        ...
        if s.startswith("drop "):
            assert s.startswith(("drop trigger if exists", "drop policy if exists")), (
                f"unexpected drop statement in 064: {s!r}"
            )
    ...
    for existing in ("profiles", "skills", "student_profiles"):
        assert f"alter table {existing} " not in sql, f"064 must not ALTER {existing}"


def test_migration_064_reuses_set_updated_at_without_redefining_it():
    sql = _sql_without_comments(MIGRATION_064)
    assert "execute procedure public.set_updated_at()" in sql
    assert "create or replace function public.set_updated_at" not in sql, (
        "set_updated_at() is defined in 012_student_profiles.sql -- 064 must reuse, not redefine it"
    )
...
def test_no_historical_migration_was_modified_by_this_phase():
    """007_learning.sql stays the placeholder it always was; 064 is the
...
def test_seed_file_exists_and_targets_only_the_learning_schema():
    assert SEED_064.is_file(), f"expected {SEED_064} to exist"
    sql = _sql_without_comments(SEED_064).lower()
...
def test_seed_has_a_reasonable_number_of_resources():
    sql = _sql_without_comments(SEED_064)
