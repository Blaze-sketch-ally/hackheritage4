...
`internship_skills`, database/migrations/053_internships.sql).
...
`internships`' policies (053_internships.sql) scope every write to
...
def create_internship(
    ...
    default (e.g. `stipend_currency not null default 'INR'`) actually get
    that default instead of violating its NOT NULL constraint. A draft
    with no stipend decided yet is exactly the case this page exists
    for, so this must not 500."""
