"""Business logic for the STUDENT side of event discovery.
...
on rather than re-implements (database/migrations/024_industry_workshops.sql):

* `industry_workshops`: "Authenticated users can view published workshops"
  -- a student only ever sees `status = 'PUBLISHED'` rows. A
  DRAFT/CLOSED/ARCHIVED workshop is invisible (reads return no row ->
  callers 404). The owner-only "Industry can manage their own workshops"
  policy is `to authenticated ... using (auth.uid() = industry_id AND
...
registration table in the schema, so there is no register/unregister
logic here to write.
"""
...
def _fetch_organizers(client: Client, industry_ids: list[str]) -> dict[str, dict]:
    """Batch-load company display info for a set of workshop owners.
...
def list_events(
    ...
    *,
    ...
    """Published industry workshops, normalized, soonest start first (rows
    with no start date last, then newest-created). RLS already restricts
    every row to `status = 'PUBLISHED'`; the explicit
    `.eq("status", "PUBLISHED")` is defence in depth."""
    query = client.table("industry_workshops").select(_SELECT).eq("status", "PUBLISHED")
    ...
    # Soonest start first; workshops with no start date sort last.
    shaped.sort(key=lambda e: (e.get("start_date") is None, e.get("start_date") or ""))
    return shaped


def get_event(client: Client, event_id: str) -> dict | None:
    """One published workshop, normalized, or None (callers turn None into
    a 404 -- a nonexistent workshop and a DRAFT/CLOSED/ARCHIVED one are
    indistinguishable to a student, even one who knows the UUID)."""
    response = (
