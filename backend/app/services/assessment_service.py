...
def list_active_assessments(client: Client) -> list[dict]:
    """Every active PRODUCTION assessment visible to the caller,
    unfiltered by skill.
    ...
    explicit .eq() here is defense in depth. Kept as a building block /
    admin-style read -- the student-facing list endpoint uses
    list_assessments_for_student() instead, which additionally narrows to
    the caller's own selected skills (see that function).

    Phase 3E: also filters source = 'PRODUCTION' (065_assessment_source_
    metadata.sql) so QA/live-fixture assessments never leak into any
    student-facing surface. Every catalog helper below
    (list_assessments_for_skill_ids, and via it list_assessments_for_student)
    re-applies the same source filter, and
    app.services.student_recommendation_service.recommend_assessments()
    inherits it through list_active_assessments -- source filtering lives
    at the query layer, not RLS.
    """
    response = (
...
def list_assessments_for_skill_ids(client: Client, skill_ids: list[str]) -> list[dict]:
    """Active PRODUCTION assessments whose skill_id is in `skill_ids`,
    ordered by created_at. An empty `skill_ids` short-circuits to [] and
    never issues a query -- PostgREST's `in.()` with an empty set is
    ambiguous and must not be relied on to mean "match nothing".

    Phase 3E source filter is applied here too: the student-facing
    skill-filtered list is (active AND source = 'PRODUCTION' AND
    skill in selected), never a QA/fixture assessment."""
    if not skill_ids:
        return []
    response = (
        client.table("assessments")
...
def list_assessments_for_student(client: Client, student_id: str) -> list[dict]:
    """The assessments a student may actually see: active PRODUCTION
    assessments for the skills that student has selected, and nothing
    else.
