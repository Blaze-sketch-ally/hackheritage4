"""Business logic for Industry training (`industry_training`,
...
`industry_training`'s policies (023_industry_training.sql) scope every
...
`status` is never editable through `update_training`.
"""
...
# Fields that must be present before a training record can be published.
...
class PublishValidationError(Exception):
    """The training record is missing fields required to publish."""
...
class InvalidStatusTransitionError(Exception):
    ...
    def __init__(self, current: str, target: str) -> None:
        ...
        self.target = target
        super().__init__(f"Cannot move a training record from {current} to {target}.")
...
def list_trainings(
    ...
    *,
    ...
    """The caller's own training records (every status by default), newest
    change first. Optional exact `status` filter and case-insensitive
    title `search`."""
    query = client.table("industry_training").select(_SELECT).eq("industry_id", industry_id)
    if status:
...
def get_training(client: Client, industry_id: str, training_id: str) -> dict | None:
    """One of the caller's own training records, or None -- callers turn
    None into a 404, so another Industry account's record is
...
def create_training(client: Client, industry_id: str, data: dict) -> dict:
    ...
    response = client.table("industry_training").insert(payload).execute()
    new_id = response.data[0]["id"]

    row = get_training(client, industry_id, new_id)
    if row is None:
        raise RuntimeError("training row could not be read back after create.")
    return row
...
def update_training(
    ...
    """Edit the caller's own training record. `status` is never touched
    here."""
    existing = get_training(client, industry_id, training_id)
    if existing is None:
        return None
    ...
    return get_training(client, industry_id, training_id)


def publish_training(client: Client, industry_id: str, training_id: str) -> dict | None:
...
def close_training(client: Client, industry_id: str, training_id: str) -> dict | None:
    """PUBLISHED -> CLOSED (stop accepting new enrollment interest)."""
    return _transition(client, industry_id, training_id, "CLOSED", _CLOSE_FROM)


def archive_training(client: Client, industry_id: str, training_id: str) -> dict | None:
    """DRAFT/PUBLISHED/CLOSED -> ARCHIVED. This is the closest thing to a
    delete -- rows are never physically removed."""
    return _transition(client, industry_id, training_id, "ARCHIVED", _ARCHIVE_FROM)
