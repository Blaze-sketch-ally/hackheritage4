...
on rather than re-implements (database/migrations/031_industry_mentorship.sql,
026_industry_profiles.sql):

* `industry_mentorship`: any authenticated user may read published
  opportunities -- a student only ever sees `status = 'PUBLISHED'` rows.
  A DRAFT/CLOSED/ARCHIVED row is invisible (reads return no row ->
  callers 404). The owner-only write policies never match a student, so
  a student has no write path to this table at all.
* `industry_profiles`: any authenticated user may read company display
  info only, never `profiles`.
