"""Business logic for Industry jobs (`jobs` + `job_skills`,
database/migrations/019_jobs.sql).

Mirrors app.services.internship_service: every function takes an
...
`jobs`' policies (019_jobs.sql) scope every write to
`auth.uid() = industry_id AND public.is_industry(auth.uid())`, and
`job_skills`' policies scope writes to skills of a job the caller owns. On
top of that, every function here also filters explicitly by `industry_id`
and forces `industry_id` / `status` server-side -- a client value for
either is never trusted.
...
`status` is never editable through `update_job`.
"""
...
_SELECT = (
    ...
    "eligibility_criteria, application_deadline, status, created_at, updated_at, "
...
# before a job can be published. title/description are NOT NULL columns so
# they are always present already.
_PUBLISH_REQUIRED = ("location", "work_mode", "employment_type", "application_deadline")
...
class PublishValidationError(Exception):
    """The job is missing fields required to publish."""
...
def _shape(row: dict) -> dict:
    """Flatten the nested job_skills embed into a plain `skills` list."""
    skills = []
    for link in row.pop("job_skills", None) or []:
...
def list_jobs(
    ...
    *,
    ...
    """The caller's own jobs (every status by default), newest change
    first. Optional exact `status` filter and case-insensitive title
    `search`."""
    query = client.table("jobs").select(_SELECT).eq("industry_id", industry_id)
    if status:
...
def get_job(client: Client, industry_id: str, job_id: str) -> dict | None:
    """One of the caller's own jobs, or None -- callers turn None into a
    404, so another Industry account's job is indistinguishable from one
    that doesn't exist."""
    response = (
        client.table("jobs")
        .select(_SELECT)
...
def _dedupe_skills(skills: list[dict]) -> list[dict]:
    ...
    never reach the unique (job_id, skill_id) constraint."""
    by_id: dict[str, dict] = {}
    for entry in skills:
...
def _replace_skills(client: Client, job_id: str, skills: list[dict]) -> None:
    ...
    if deduped:
        client.table("job_skills").insert(
            [{"job_id": job_id, **s} for s in deduped]
        ).execute()
...
def create_job(client: Client, industry_id: str, data: dict, skills: list[dict]) -> dict:
    ...
    if deduped:
        client.table("job_skills").insert(
            [{"job_id": new_id, **s} for s in deduped]
        ).execute()

    row = get_job(client, industry_id, new_id)
    if row is None:
        raise RuntimeError("job row could not be read back after create.")
    return row
...
def update_job(
    ...
    """Edit the caller's own job. `status` is never touched here.
    `skills=None` leaves the skill list alone; a list replaces it."""
    existing = get_job(client, industry_id, job_id)
    if existing is None:
        return None
    ...
    if skills is not None:
        _replace_skills(client, job_id, skills)

    return get_job(client, industry_id, job_id)


def publish_job(client: Client, industry_id: str, job_id: str) -> dict | None:
...
def close_job(client: Client, industry_id: str, job_id: str) -> dict | None:
    """PUBLISHED -> CLOSED (stop accepting new applications)."""
    return _transition(client, industry_id, job_id, "CLOSED", _CLOSE_FROM)


def archive_job(client: Client, industry_id: str, job_id: str) -> dict | None:
    """DRAFT/PUBLISHED/CLOSED -> ARCHIVED. This is the closest thing to a
    delete -- rows are never physically removed (019's FKs from
    applications are ON DELETE RESTRICT, and recruitment history must
    survive)."""
    return _transition(client, industry_id, job_id, "ARCHIVED", _ARCHIVE_FROM)
