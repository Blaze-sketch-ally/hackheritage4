...
pass -- adapted to this project's naming (paired with
app.api.industry_mentorship_opportunities, the Industry-side management
API over the same `industry_mentorship` table) and given its own
non-colliding route prefix (`/student/mentorship-opportunities`, distinct
from the unrelated `/student/mentorships` -- FACULTY<->STUDENT 1:1
mentorship requests, app.api.student_mentorships). This is a genuinely
different feature: read-only discovery of INDUSTRY-published mentorship
programs, not a duplicate of the faculty mentorship request workflow.
...
(database/migrations/031_industry_mentorship.sql, unchanged): no
...
from app.schemas.student_mentorship_opportunity import (
    StudentMentorshipDetail,
    StudentMentorshipListResponse,
    StudentMentorshipSummary,
)
from app.services import student_mentorship_opportunity_service

router = APIRouter(prefix="/student/mentorship-opportunities", tags=["student-mentorship-opportunities"])
...
@router.get("", response_model=StudentMentorshipListResponse)
def list_mentorship_opportunities(
    ...
    try:
        rows = student_mentorship_opportunity_service.list_mentorships(
...
@router.get("/{mentorship_id}", response_model=StudentMentorshipDetail)
def get_mentorship_opportunity(
    ...
    client = build_user_client(current_user.access_token)
    try:
        row = student_mentorship_opportunity_service.get_mentorship(client, str(mentorship_id))
    except Exception as exc:
