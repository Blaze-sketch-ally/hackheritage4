...
Phase 1D: read-only endpoints only. Every route requires require_student()
(which itself requires get_current_user()) and reads through
build_user_client(access_token) -- never get_supabase() -- so RLS stays
the real access-control boundary. See app.services.assessment_service for
the actual queries.
"""
