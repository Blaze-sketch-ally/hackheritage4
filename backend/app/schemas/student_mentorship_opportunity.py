...
(database/migrations/031_industry_mentorship.sql). There is no
...
`industry_mentorship` row -- the same row app.api.industry_mentorship_
opportunities already manages on the Industry side.

`industry_mentorship` already carries an RLS policy letting any
authenticated user read published rows, so a STUDENT caller can already
read those rows through build_user_client(); this adapter relies on that
rather than widening any policy.

There is NO mentorship request / pairing / enrollment model anywhere in
the repository. A request table with no Industry-side responder API or
UI would be a dead-end half-feature, so this adapter does not introduce
one. It exposes `requests_available: false` and the student frontend
shows a truthful "requests aren't available yet" state. Mentorship is
strictly read-only for students; nothing here can create, update, or
delete an `industry_mentorship` row.
...
# database/migrations/031_industry_mentorship.sql -- industry_mentorship.work_mode CHECK.
