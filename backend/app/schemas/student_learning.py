...
# database/migrations/064_learning_resources.sql -- CHECK constraints
...
class MatchedGapSkill(BaseModel):
    """One aggregate skill-gap skill (an item of
    ``app.services.skill_recommendation_service.get_aggregate_skill_gaps``'s
    output) that a recommended learning resource is mapped to via
    ``learning_resource_skills.skill_id``.

    ``reason`` is that engine's own deterministic, server-authored text --
    surfaced verbatim so the UI can explain *why* the resource is
...
class LearningRecommendationListResponse(BaseModel):
    """``mode`` is always ``"AGGREGATE"`` -- recommendations come from
    app.services.skill_recommendation_service, which aggregates GAP /
    NOT_ASSESSED skills across every career role in this project's own
    catalog (there is no persisted "target role" concept here to compute
    a single-role gap from instead). The recommendation skills themselves
    are always that engine's output -- this endpoint never computes its
    own."""
