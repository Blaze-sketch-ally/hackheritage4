...
-- 019_jobs in that branch). Already applied to the live Supabase project
-- (confirmed: jobs/job_skills tables exist live with this exact shape,
-- with real data) -- this file exists so a fresh environment matches.
-- `if not exists`/`drop policy if exists` throughout makes re-running
-- this against the current live project a safe no-op.
--
-- NOTE: 006_jobs.sql is an earlier, still-unimplemented placeholder for
-- this same feature area -- left untouched per this repo's own
-- "historical migrations are never rewritten" convention. This migration
-- is the real schema, not a replacement of 006.
--
-- Same ownership reasoning as internships (053_internships.sql):
...
-- Partial index for the public "browse published jobs" query.
