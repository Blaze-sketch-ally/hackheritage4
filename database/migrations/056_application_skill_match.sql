-- Migration: 056_application_skill_match
...
-- 021_application_skill_match in that branch, referencing its own
-- 020_applications.sql -- renamed 055_applications.sql here). Additive,
-- idempotent (create or replace + revoke/grant) -- safe to run against
-- the current live project even though 055_applications.sql itself must
-- not be re-run there.
--
...
-- public.set_application_industry_id rely on (055_applications.sql).
...
-- same hardening as public.is_industry (026_industry_profiles.sql).
