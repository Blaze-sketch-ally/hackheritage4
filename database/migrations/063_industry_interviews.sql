-- Migration: 063_industry_interviews
...
-- 030_industry_interviews). Already applied to the live Supabase project
-- (confirmed: interviews exists live with real data).
--
-- Purpose: Interview Scheduling for the Industry recruitment pipeline.
-- Until now "Interviews" was only an application STATUS
-- (applications.status = 'INTERVIEW_SCHEDULED', migration 055) with no
...
--
-- An interview always hangs off an existing `applications` row (055):
...
-- BEFORE INSERT trigger, exactly like set_application_industry_id in 055.
--
...
create table if not exists interviews (
  ...
  -- applications.industry_id (055): ON DELETE RESTRICT so interview
...
-- set_application_industry_id (055_applications.sql).
...
begin
  ...
  -- pipeline (055): SHORTLISTED -> INTERVIEW_SCHEDULED. The service layer
...
-- pattern as prevent_application_identity_change (055_applications.sql).
...
-- cancellation is a status (CANCELLED), matching 055/026/027/028.
