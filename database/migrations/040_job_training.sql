...
-- (037/038/039). Two halves in one file:
...
-- own separate Internship Workspace (038) and never touch anything here.
--
-- ============================================================
...
--   set_workspace_derived_ids (038) / set_application_industry_id (020).
...
-- posting closes -- identical stance to student_can_access_program (038),
-- which never reads internships.status.
--
-- ============================================================
-- Conventions reused (001-039)
-- ============================================================
...
--   owns_internship_program / student_can_access_program (037/038).
...
--   (020/027/028/030/037/038 precedent). Authoring mistakes are corrected
...
-- ============================================================
-- The 037 live-push bug was a `language sql` helper created BEFORE its
...
-- exactly like public.owns_internship_program (037). Reads the ownership
...
-- vocabulary" pattern as internship module_items (037). VIDEO / PDF / LINK
...
-- normalized shape as internship program_skills (037). Mutable child
...
-- program_assignments (037). MVP quiz submissions are reviewed manually.
...
create table if not exists job_program_assignments (
  ...
  -- set_program_assignment_program_id (037) / set_application_industry_id (020).
  program_id uuid not null references job_programs (id) on delete cascade,
...
create table if not exists job_training_enrollments (
  ...
  -- mirrors internship_workspaces (038): student_id CASCADE (a student
...
-- OLD-vs-NEW pattern as prevent_workspace_identity_change (038) /
...
-- aside. Mirrors enforce_workspace_status_transitions (038).
...
-- construction as the internship workspace provisioning INSERT policy (038).
...
-- public.student_can_access_program (038), which never reads
...
-- NOT on jobs.status. Same shape as the 038 student-read policies for the
-- 037 program tables.
...
-- non-destructive. Exactly the DO-block name-resolution shape 039 used
...
--
-- Ordering note: this section assumes migration 039 has already run (it is
-- lower-numbered, so a forward replay guarantees that). It re-adds the
-- same two constraint names 039 uses, extended by one value each; the
-- final value sets are the 039 sets PLUS the job-training value.
