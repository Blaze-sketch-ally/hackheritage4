-- Migration: 057_application_applicant_names
...
-- (055_applications.sql), and Industry has no RLS read access to
...
-- collaboration_counterparty_names (a prior migration in this repo)
-- already climbs for the same
...
-- precedent.
--
-- Adopted from a collaborator branch during the cross-branch
-- integration pass, renumbered to fit this repo's migration sequence
-- (originally 036_application_applicant_names in that branch). Additive,
-- idempotent (create or replace + revoke/grant) -- safe to run against
-- the current live project.
--
-- Hardened exactly like this repo's own SECURITY DEFINER read helpers
-- (e.g. public.is_industry, 013_harden_is_student.sql):
