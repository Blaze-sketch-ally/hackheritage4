-- Migration: 084_forbid_internship_job_deletes
-- Purpose: extend the hard-delete lockdown from
-- 033_forbid_industry_record_deletes.sql to the two Industry posting
-- tables that carry the identical exposure -- `internships`
-- (053_internships.sql) and `jobs` (054_jobs.sql) -- under the same
-- product decision:
--
--     HARD DELETE MUST NOT BE POSSIBLE FOR INDUSTRY RECORDS.
--
-- Ported from a collaborator's parallel migration lineage (their
-- 028_forbid_internship_job_deletes.sql, same fix, renumbered here to
-- continue this branch's own numbering with no historical migration
-- file edited).
--
-- Both tables currently grant their owner one `for all` policy
-- ("Industry can manage their own internships" / "... jobs"), and
-- `for all` includes DELETE. The application never deletes a posting:
-- internship_service / job_service only ever call .insert()/.update() on
-- `internships` / `jobs`, there is no DELETE API route, and the
-- lifecycle is status-only (DRAFT -> PUBLISHED -> CLOSED -> ARCHIVED).
-- But the `for all` policy still lets an owner issue a direct
-- `DELETE FROM internships WHERE ...` against Supabase's REST endpoint
-- with their own JWT.
--
-- Fix (identical shape to migration 033): replace each single `for all`
...
--   * internship_skills (053) and job_skills (054) keep their existing
...
--     policies -- untouched.
--   * Recruitment (055_applications.sql), Analytics, and
--     Student/Faculty/Institution RLS -- untouched.
--
-- Idempotent: every statement is `drop policy if exists` + `create
-- policy`, matching every other migration in this project.

-- ============================================================
-- 053_internships -- public.internships
-- ============================================================
