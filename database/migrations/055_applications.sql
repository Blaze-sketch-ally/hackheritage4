...
-- 053_internships.sql and 054_jobs.sql (industry_id resolution below reads
-- both).
--
-- Adopted from a collaborator branch during the cross-branch integration
-- pass, renumbered to fit this repo's migration sequence (originally
-- 020_applications in that branch).
--
-- ****************************************************************
-- DO NOT RUN THIS MIGRATION AGAINST THE CURRENT LIVE SUPABASE PROJECT.
-- It has ALREADY been applied there (confirmed: the live `applications`
-- table already has exactly this shape -- student_id/industry_id/
-- opportunity_type/internship_id/job_id/status/cover_note/match_score/
-- applied_at -- and already carries real recruitment data). This file's
-- own `drop table if exists applications cascade` (below) is DESTRUCTIVE:
-- re-running it against the current project would permanently delete
-- that real data. This file exists ONLY so a fresh environment (a new
-- Supabase project spun up from this migration history) ends up with the
-- same schema -- it is historical/documentation for this repo, not an
-- instruction to re-run it here.
--
-- This repo's earlier 024_opportunities_and_applications.sql migration
-- created a DIFFERENT, now-retired `applications` shape (keyed by a
-- generalized `opportunities`/`opportunity_id`, not internship_id/job_id).
-- That schema is what this migration's own drop-and-recreate below
-- superseded on the live project at some point before this integration
-- pass -- explaining exactly why the live table already has this shape.
-- 024's `opportunities`/`opportunity_skill_requirements` tables and the
-- application code built on them (app.api.opportunities,
-- the opportunity-scoped parts of app.services.application_service) are
-- retired by this same integration pass -- see this pass's own commit/PR
-- description for the full reasoning (live usage evidence showed this
-- internship_id/job_id-keyed schema, not 024's, was the one actually in
-- active use).
-- ****************************************************************
--
...
--     (student_skills, assessment attempts): a student is allowed to take
--     their own rows with them when their account is deleted.
...
create table applications (
  ...
  cover_note text,
  -- Filled later by the deterministic matching service (app.services.match_service).
...
-- (053_internships.sql / 054_jobs.sql) -- publication state lives there,
-- not on this table.
