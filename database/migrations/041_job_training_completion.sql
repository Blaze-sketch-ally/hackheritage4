-- Migration: 041_job_training_completion
...
-- ============================================================
-- J1 (040_job_training.sql) delivered job_programs + content + the
...
-- (039_workspace_submissions_completion.sql) and adapting it:
...
--   4. Two enrollment-ownership RLS helpers (040 shipped none).
--
-- ============================================================
...
--   the 020 / 038 / 040 "derive-and-freeze" precedent.
...
--   ON DELETE CASCADE (040). Enrollments are never hard-deleted, so this
...
-- * NO student_notifications change -- 040 already widened its CHECKs to
--   allow type 'JOB_TRAINING' and related_entity_type
--   'JOB_TRAINING_ENROLLMENT'. This migration ASSUMES 040 has run (it is
--   lower-numbered, so a forward replay guarantees that).
--
-- ============================================================
-- Conventions reused (001-040)
-- ============================================================
...
--   NO DELETE policy (020/027/028/030/037/038/039/040 precedent).
-- * Certificate number generation: RFC 4648 base32 of 8 CSPRNG bytes via
--   md5(gen_random_uuid()...) + get_byte() + numeric -- no pgcrypto, no
--   gen_random_bytes, no bit-string cast. Exact copy of 039's approach,
...
--      student_owns_job_training_enrollment()        job_training_enrollments (040)
...
-- ============================================================
-- 040 shipped owns_job_program / student_can_access_job_program (keyed on
-- program_id) but no enrollment-keyed helper. These two mirror
-- student_owns_workspace / industry_owns_workspace (038). SECURITY
...
-- set_internship_completion_verifier (039).
--
...
-- guard on stipend_disbursements (039). No service_role escape hatch --
...
-- RFC 4648 base32 of 8 CSPRNG bytes -- exact copy of 039's
...
-- supplied. Mirrors set_internship_certificate_derived_ids (039).
...
-- revocation state. Mirrors prevent_internship_certificate_tamper (039).
