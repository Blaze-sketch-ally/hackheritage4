...
-- tracking lives in 055_applications.sql, not here.
--
...
-- 018_internships in that branch, referencing its own 020_applications.sql
-- and 016_skill_gap.sql -- both renamed here too, see 055/052-and-onward).
-- Already applied to the live Supabase project (confirmed: internships/
-- internship_skills tables exist live with this exact shape, with real
-- data) -- this file exists so a fresh environment matches. `if not
-- exists`/`drop policy if exists` throughout makes re-running this
-- against the current live project a safe no-op.
--
-- NOTE: 005_internships.sql is an earlier, still-unimplemented placeholder
-- for this same feature area -- left untouched per this repo's own
-- "historical migrations are never rewritten" convention. This migration
-- is the real schema, not a replacement of 005.
--
...
-- (026_industry_profiles.sql) is created lazily -- same reasoning already
-- applied elsewhere in this schema (e.g. student_skills.student_id,
-- 003_skills.sql). An industry user must be able to post before ever
-- touching their company profile form.
