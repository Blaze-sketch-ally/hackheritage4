...
/**
 * Role-generic version of components/student/student-header.tsx -- same
 * UI/behavior, with the two role-specific routes (profile/settings)
 * passed in instead of hard-coded, so Faculty/Industry/Institution can
 * share one implementation instead of three near-identical copies.
 * Student keeps its own StudentHeader unchanged (not worth the churn/risk
 * of migrating a working page for a naming consolidation).
 */
export function AppHeader({
  ...
  const roleLabel =
    profile.role && profile.role in ROLE_LABELS ? ROLE_LABELS[profile.role as PublicRole].title : "";
    ...
    <header className="sticky top-0 z-30 flex h-14 shrink-0 items-center gap-3 border-b bg-card/80 px-4 backdrop-blur supports-backdrop-filter:bg-card/60 sm:px-6">
      ...
      <div className="relative hidden max-w-sm flex-1 sm:block">
        <Search
          ...
          aria-hidden="true"
        />
        <Input placeholder={searchPlaceholder} className="h-9 pl-8" disabled aria-label="Search (coming soon)" />
      </div>
