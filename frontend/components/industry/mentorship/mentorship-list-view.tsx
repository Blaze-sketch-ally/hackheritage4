...
import { AlertCircle, Handshake, Plus, RefreshCw, Search } from "lucide-react";
...
import { MentorshipCard } from "@/components/industry/mentorship/mentorship-card";

type LoadState =
  | { status: "loading" }
  | { status: "error"; error: ApiError }
  | { status: "ready"; mentorships: IndustryMentorship[] };
...
export function MentorshipListView() {
    ...
    getMentorshipOpportunities()
      ...
      .catch((err) => {
        ...
        setState({
          status: "error",
          error:
            err instanceof ApiError ? err : new ApiError(0, "Could not load your mentorship opportunities."),
        });
      });
    return () => {
      cancelled = true;
  ...
  const visible = useMemo(() => {
    ...
    return state.mentorships.filter((mentorship) => {
      const matchesStatus = statusFilter === "all" || mentorship.status === statusFilter;
      const matchesSearch = !query || mentorship.title.toLowerCase().includes(query);
  ...
  async function runAction(id: string, action: LifecycleAction) {
    ...
    try {
      ...
      setState((prev) =>
        prev.status === "ready"
          ? { ...prev, mentorships: prev.mentorships.map((m) => (m.id === id ? updated : m)) }
          : prev,
      );
      setActionSuccess(
        action === "publish"
          ? "Mentorship opportunity published."
          : action === "close"
            ? "Mentorship opportunity closed."
            : "Mentorship opportunity archived.",
      );
    } catch (err) {
      setActionError(
  ...
  const statusOptions = [
    ...
    ...MENTORSHIP_STATUSES.map((s) => ({ value: s, label: MENTORSHIP_STATUS_LABELS[s] })),
  ];
    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Create and manage your mentorship opportunities.</p>
        </div>
        <Button render={<Link href="/industry/mentorship/create" />}>
          <Plus className="size-4" /> Create Mentorship Opportunity
        </Button>
      </div>
      ...
      {state.status === "loading" ? (
        <Card>
          <CardContent className="py-10 text-center text-sm text-muted-foreground" aria-busy="true">
            Loading your mentorship opportunities…
          </CardContent>
        </Card>
      ) : null}
            ...
            <div>
              <p className="font-medium">
                {state.error.status === 401
                  ...
                  : "Could not load your mentorship opportunities."}
              </p>
      ...
      {state.status === "ready" ? (
        state.mentorships.length === 0 ? (
          <EmptyState
            icon={Handshake}
            title="No mentorship opportunities yet"
            description="Create your first mentorship opportunity and save it as a draft."
            actionLabel="+ Create Mentorship Opportunity"
            onAction={() => router.push("/industry/mentorship/create")}
          />
        ) : (
          <>
            ...
            {visible.length === 0 ? (
              <EmptyState icon={Search} title="No mentorship opportunities match your filters" />
            ) : (
              <div className="space-y-3">
                {visible.map((mentorship) => (
                  <MentorshipCard
                    ...
                    pending={pending?.id === mentorship.id}
                    onPublish={() => setConfirming({ id: mentorship.id, action: "publish" })}
                    onClose={() => setConfirming({ id: mentorship.id, action: "close" })}
                    onArchive={() => setConfirming({ id: mentorship.id, action: "archive" })}
                  />
                ))}
              </div>
            )}
