...
function toFormState(initial?: IndustryMentorship) {
  return {
    title: initial?.title ?? "",
    description: initial?.description ?? "",
    location: initial?.location ?? "",
    workMode: (initial?.work_mode ?? null) as MentorshipWorkMode | null,
    ...
    applicationDeadline: initial?.application_deadline?.slice(0, 10) ?? "",
...
export function MentorshipForm({
  ...
  mode: "create" | "edit";
  initial?: IndustryMentorship;
  submitting: boolean;
  error: string | null;
  onSubmit: (data: MentorshipCreate) => void;
  onCancel: () => void;
}) {
  ...
  function validate(): boolean {
    ...
    if (!form.location.trim()) errors.location = "A location is required.";
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    const data: MentorshipCreate = {
      title: form.title.trim(),
      description: form.description.trim(),
      location: form.location.trim(),
      work_mode: form.workMode as MentorshipWorkMode,
        ...
        <CardContent className="space-y-4">
          <div className="space-y-1.5">
            ...
            <Input
              ...
              maxLength={200}
              placeholder="e.g. Frontend Career Mentorship"
              disabled={submitting}
          ...
          <div className="space-y-1.5">
            ...
            <textarea
              ...
              rows={5}
              maxLength={10000}
              placeholder="What mentees will get out of this, format, focus areas..."
              disabled={submitting}
      ...
      <Card>
        ...
        <CardContent className="grid gap-4 sm:grid-cols-2">
          <div className="space-y-1.5">
            ...
            />
            <FieldError id={`${ids.location}-error`} message={fieldErrors.location} />
          </div>
          <div className="space-y-1.5">
            ...
            <Select
              ...
              onValueChange={(next) =>
                set("workMode", next === UNSET ? null : (next as MentorshipWorkMode))
              }
              disabled={submitting}
            >
              <SelectTrigger id={ids.workMode} className="w-full" aria-invalid={!!fieldErrors.workMode}>
              ...
              <SelectContent>
                ...
                {MENTORSHIP_WORK_MODES.map((m) => (
                  <SelectItem key={m} value={m}>
                    {MENTORSHIP_WORK_MODE_LABELS[m]}
                  </SelectItem>
                ))}
              </SelectContent>
            </Select>
            <FieldError id={`${ids.workMode}-error`} message={fieldErrors.workMode} />
          </div>
          <div className="space-y-1.5">
