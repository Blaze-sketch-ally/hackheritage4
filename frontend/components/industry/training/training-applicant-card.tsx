...
import { TrainingApplicationStatusActions } from "@/components/industry/training/training-application-status-actions";
import { TrainingApplicationStatusBadge } from "@/components/industry/training/training-application-status-badge";
import { ensureWorkspace } from "@/lib/industry/participation";
import {
  trainingApplicantDisplayName,
...
/** One applicant row for a Training's Applicants view. Shows only
 * server-resolved, human-readable identity (via
 * public.training_applicant_profiles) -- never a raw student_id. */
export function TrainingApplicantCard({
  application,
  pending,
  onPick,
}: {
  ...
  const name = trainingApplicantDisplayName(application);
  ...
  async function openWorkspace() {
    ...
    try {
      const ws = await ensureWorkspace({ kind: "TRAINING", training_application_id: application.id });
    ...
    <Card>
      <CardContent className="space-y-3">
        <div className="flex items-start justify-between gap-3">
          ...
          <TrainingApplicationStatusBadge status={application.status} />
        </div>
        ...
        <div className="flex flex-wrap items-center justify-between gap-2 pt-1">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <TrainingApplicationStatusActions status={application.status} pending={pending} onPick={onPick} />
          </div>
        </div>
      </CardContent>
    </Card>
