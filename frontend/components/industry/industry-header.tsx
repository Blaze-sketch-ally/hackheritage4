...
export function IndustryHeader({ profile, onMenuClick }: { profile: Profile; onMenuClick: () => void }) {
  const router = useRouter();
  const displayName = profile.full_name || profile.username || "Industry";
  const roleLabel =
  ...
  return (
    <header className="sticky top-0 z-30 flex h-14 shrink-0 items-center gap-3 border-b bg-card/80 px-4 backdrop-blur supports-backdrop-filter:bg-card/60 sm:px-6">
      ...
      <div className="relative hidden max-w-sm flex-1 sm:block">
        ...
        />
        <Input
          placeholder="Search internships, jobs, applicants..."
          className="h-9 pl-8"
          disabled
      ...
      <div className="flex flex-1 items-center justify-end gap-1.5 sm:flex-none">
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end">
            ...
            <DropdownMenuSeparator />
            <DropdownMenuItem render={<Link href="/industry/profile" />}>
              <UserIcon /> Profile
            </DropdownMenuItem>
            <DropdownMenuItem render={<Link href="/industry/settings" />}>
              <Settings /> Settings
            </DropdownMenuItem>
            <DropdownMenuSeparator />
