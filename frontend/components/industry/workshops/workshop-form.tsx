...
function toFormState(initial?: IndustryWorkshop) {
  return {
    title: initial?.title ?? "",
    description: initial?.description ?? "",
    location: initial?.location ?? "",
    workMode: (initial?.work_mode ?? null) as WorkshopWorkMode | null,
    durationDays: initial?.duration_days?.toString() ?? "",
...
export function WorkshopForm({
  ...
  mode: "create" | "edit";
  initial?: IndustryWorkshop;
  submitting: boolean;
  error: string | null;
  onSubmit: (data: WorkshopCreate) => void;
  onCancel: () => void;
}) {
  ...
  function validate(): boolean {
    ...
    if (form.durationDays.trim()) {
      ...
      if (!Number.isInteger(n) || n < 1 || n > 365) {
        errors.durationDays = "Enter a whole number of days between 1 and 365.";
      }
    }
        ...
        <CardContent className="space-y-4">
          <div className="space-y-1.5">
            ...
            <Input
              ...
              maxLength={200}
              placeholder="e.g. Intro to Git Workshop"
              disabled={submitting}
          ...
          <div className="space-y-1.5">
            ...
            <textarea
              ...
              rows={5}
              maxLength={10000}
              placeholder="Agenda, format, what attendees will learn..."
              disabled={submitting}
        ...
        <CardContent className="grid gap-4 sm:grid-cols-2">
          ...
          <div className="space-y-1.5">
            ...
            <Select
              ...
              onValueChange={(next) =>
                set("workMode", next === UNSET ? null : (next as WorkshopWorkMode))
              }
              disabled={submitting}
            >
              ...
              <SelectContent>
                ...
                {WORKSHOP_WORK_MODES.map((m) => (
                  <SelectItem key={m} value={m}>
                    {WORKSHOP_WORK_MODE_LABELS[m]}
                  </SelectItem>
                ))}
              </SelectContent>
            </Select>
          ...
          <div className="space-y-1.5">
            <Label htmlFor={ids.durationDays}>Duration (days)</Label>
            <Input
              id={ids.durationDays}
              inputMode="numeric"
              ...
              placeholder="e.g. 1"
              disabled={submitting}
              aria-invalid={!!fieldErrors.durationDays}
            />
            <FieldError id={`${ids.durationDays}-error`} message={fieldErrors.durationDays} />
          </div>
          <div className="space-y-1.5">
