...
/** Add or edit an assignment (ASSIGNMENT / QUIZ / PROJECT -- one normalized
 * job_program_assignments row, no quiz engine). Mirrors the
 * job_program_assignments_repo_kind_consistent CHECK: "requires a repo"
 * forces a REPO/MIXED submission kind. There is no delete -- the parent
 * hides an assignment via is_published. Mirrors
 * internship-program/assignment-form.tsx. */
export function JobAssignmentForm({
  ...
  busy: boolean;
  onSubmit: (data: JobAssignmentInput & { title: string }) => Promise<void>;
  onCancel: () => void;
}) {
    ...
    <div className="flex flex-col gap-3 rounded-lg border border-dashed p-3">
      <div className="grid gap-3 sm:grid-cols-[1fr_9rem]">
        <div className="space-y-1.5">
          <Label htmlFor="job-a-title">Assignment title</Label>
          <Input
            id="job-a-title"
            value={title}
        ...
        <div className="space-y-1.5">
          <Label htmlFor="job-a-type">Type</Label>
          <select
            id="job-a-type"
      ...
      <div className="space-y-1.5">
        <Label htmlFor="job-a-instructions">Instructions</Label>
        <Textarea
          id="job-a-instructions"
          value={instructions}
      ...
      <div className="space-y-1.5">
        <Label htmlFor="job-a-kind">What the candidate submits</Label>
        <select
          id="job-a-kind"
          className={SELECT_CLASS}
          value={effectiveKind}
      ...
      <div className="grid gap-3 sm:grid-cols-3">
        <div className="space-y-1.5">
          <Label htmlFor="job-a-due">Due (days after start)</Label>
          <Input
            id="job-a-due"
            type="number"
            min={0}
        ...
        <div className="space-y-1.5">
          <Label htmlFor="job-a-score">Max score</Label>
          <Input
            id="job-a-score"
            type="number"
            min={1}
        ...
        <div className="space-y-1.5">
          <Label htmlFor="job-a-skill">Linked skill</Label>
          <select
            id="job-a-skill"
            className={SELECT_CLASS}
            value={linkedSkillId}
