...
/** Program skills are picked from the job's own recruitment skills
 * (job_skills) as a suggestion list. Each selected skill is Required or
 * Optional. "Save skills" sends the full replace-set. Mirrors
 * internship-program/program-skills-editor.tsx. */
export function JobProgramSkillsEditor({
  skills,
  availableSkills,
  busy,
  onSave,
    ...
    <Card>
      <CardHeader>
        ...
        <p className="text-sm text-muted-foreground">
          Choose from this job&apos;s required skills. Add a skill to the job posting first if
          it isn&apos;t listed.
        </p>
      </CardHeader>
      <CardContent className="flex flex-col gap-4">
        {availableSkills.length === 0 ? (
          <p className="text-sm text-muted-foreground">
            This job has no skills yet. Add them on the job posting.
          </p>
        ) : (
          <>
