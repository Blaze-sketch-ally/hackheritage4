...
import { archiveJob, closeJob, getJobs, publishJob } from "@/lib/industry/jobs";
import { JOB_STATUSES, JOB_STATUS_LABELS, type Job } from "@/types/job";
import { JobCard } from "@/components/industry/jobs/job-card";
...
export function JobsListView() {
  ...
  useEffect(() => {
    ...
    getJobs()
      ...
      .catch((err) => {
        ...
        setState({
          status: "error",
          error: err instanceof ApiError ? err : new ApiError(0, "Could not load your jobs."),
        });
      });
    return () => {
      cancelled = true;
  ...
  async function runAction(id: string, action: LifecycleAction) {
    ...
    try {
      ...
      setState((prev) =>
        prev.status === "ready"
          ? { ...prev, jobs: prev.jobs.map((job) => (job.id === id ? updated : job)) }
          : prev,
      );
      setActionSuccess(
        action === "publish"
    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Create and manage your job opportunities.</p>
        </div>
        <Button render={<Link href="/industry/jobs/create" />}>
      ...
      {state.status === "ready" ? (
        state.jobs.length === 0 ? (
          <EmptyState
            icon={Briefcase}
            title="No jobs yet"
            description="Create your first job opportunity and save it as a draft."
            actionLabel="+ Create Job"
            onAction={() => router.push("/industry/jobs/create")}
          />
        ) : (
          <>
            ...
            {visible.length === 0 ? (
              <EmptyState icon={Search} title="No jobs match your filters" />
            ) : (
              <div className="space-y-3">
                {visible.map((job) => (
                  <JobCard
                    ...
                    onPublish={() => setConfirming({ id: job.id, action: "publish" })}
                    onClose={() => setConfirming({ id: job.id, action: "close" })}
                    onArchive={() => setConfirming({ id: job.id, action: "archive" })}
                  />
                ))}
              </div>
            )}
