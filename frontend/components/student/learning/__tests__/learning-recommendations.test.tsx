...
function payload(
  ...
  return { mode: "AGGREGATE", recommendations: [recommendation()], ...overrides };
}
...
describe("LearningRecommendations", () => {
  ...
  it("shows the aggregate skill-gap subtitle", async () => {
    mocks.getRecommendedLearningResources.mockResolvedValueOnce(payload({ mode: "AGGREGATE" }));
    render(<LearningRecommendations />);
