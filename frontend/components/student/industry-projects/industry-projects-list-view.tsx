...
import { AlertCircle, Building2, CalendarDays, CheckCircle2, FolderKanban, Inbox, MapPin, RefreshCw } from "lucide-react";
...
import { listMyProjectApplications, listProjects, withdrawProjectApplication } from "@/lib/student/industry-projects";
...
type MineState =
  | { status: "loading" }
  | { status: "error"; message: string }
  | { status: "ready"; applications: ProjectApplication[] };
...
const WITHDRAWABLE = new Set(["APPLIED", "SHORTLISTED"]);
const WORKSPACE_ELIGIBLE = new Set(["SELECTED", "ACTIVE", "COMPLETED"]);

export function IndustryProjectsListView() {
  ...
  useEffect(() => {
    let cancelled = false;
    listProjects({ search }).then(
      ({ projects }) => !cancelled && setBrowseState({ status: "ready", projects }),
      (err) =>
        !cancelled &&
        setBrowseState({
          status: "error",
          message: err instanceof ApiError ? err.message : "Could not load projects.",
        }),
    );
    return () => {
      cancelled = true;
  ...
  useEffect(() => {
    if (tab !== "mine") return;
    let cancelled = false;
    listMyProjectApplications().then(
  ...
  async function handleWithdraw(id: string) {
    ...
    try {
      const updated = await withdrawProjectApplication(id);
      setMineState((s) =>
        s.status === "ready"
    ...
    <div className="space-y-4">
      ...
      {tab === "browse" && (
        <>
          ...
          {browseState.status === "ready" && browseState.projects.length === 0 && (
            <EmptyState icon={Inbox} title="No projects available" description="Check back soon for new projects." />
          )}
          {browseState.status === "ready" && browseState.projects.length > 0 && (
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
              {browseState.projects.map((p) => (
                <Card key={p.id} className="flex flex-col">
                  ...
                  <CardFooter>
                    <Button
                      className="w-full"
                      render={<Link href={`/student/industry-projects/${p.id}`} />}
                      nativeButton={false}
                    >
                      <FolderKanban /> View Details
                    </Button>
                  </CardFooter>
                </Card>
              ))}
      ...
      {tab === "mine" && (
        <>
          ...
          {mineState.status === "ready" && mineState.applications.length === 0 && (
            <EmptyState icon={Inbox} title="No applications yet" description="Apply to a project to see it here." />
          )}
          {mineState.status === "ready" && mineState.applications.length > 0 && (
                ...
                <Card key={a.id}>
                  <CardContent className="space-y-2">
                    <div className="flex items-start justify-between gap-2">
                      <p className="font-medium">{a.project?.title ?? "(project unavailable)"}</p>
                      <Badge variant="ghost">{PROJECT_APPLICATION_STATUS_LABELS[a.status]}</Badge>
                    </div>
                    {a.applied_at && (
                    ...
                    <div className="flex gap-2 pt-1">
                      <Button
                        size="sm"
                        variant="outline"
                        render={<Link href={`/student/industry-projects/${a.project_id}`} />}
                      >
                        View Project
                      </Button>
                      {WORKSPACE_ELIGIBLE.has(a.status) && (
                        <Button
                          size="sm"
                          render={<Link href={`/student/industry-projects/${a.project_id}/workspace`} />}
                        >
                          Open Workspace
                        </Button>
                      )}
      ...
      <ConfirmationDialog
        ...
        description="You can't undo this. You may be able to re-apply if the project is still open."
