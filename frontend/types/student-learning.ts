...
/** Mirrors backend `LearningRecommendationListResponse`. `mode` is always
 * `"AGGREGATE"` -- recommendations come from skill_recommendation_service,
 * which aggregates skill gaps across every career role in the catalog
 * (there is no persisted "target role" concept here). */
